...
class MonitorService : Service() {
    ...
    private fun evaluateAndRing(st: GlucoseRepository.State) {
        ...
        val book = AlarmEngine.Bookkeeping(
            store.firedAlert, store.lastAlarmMs, store.lastAlarmValue, store.lastJudgedReadingMs
        )
        val dec = AlarmEngine.decide(alert, book, now, readingMs = st.current?.timestampMs ?: 0L)
        // Persist the (possibly updated) bookkeeping so the foreground UI continues the same episode
        // — and, via lastJudgedMs, so the UI knows this reading has already been ruled on and does
        // not sound it a second time when the app is opened.
        store.firedAlert = dec.book.firedKind
        store.lastAlarmMs = dec.book.lastAlarmMs
        store.lastAlarmValue = dec.book.lastAlarmValue
        store.lastJudgedReadingMs = dec.book.lastJudgedMs
        if (dec.recovered) {
            store.ackAlert = null
    ...
    companion object {
        ...
        const val ACTION_STOP = "com.drclaude.MONITOR_STOP"
        const val ACTION_POKE = "com.drclaude.MONITOR_POKE"
