...
/** One followed person in the Profile switcher. [glucose] / [trend] let a PINNED device show that
 *  person's reading in a read-only peek without ever leaving its own record. */
data class PatientChoice(
    val patientId: String,
    val label: String,
    val active: Boolean,
    val glucose: Int? = null,
    val readingMs: Long = 0L,
)
