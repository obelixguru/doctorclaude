...
import com.drclaude.ai.VoiceInput
import com.drclaude.data.AlarmEngine
import com.drclaude.data.AlarmPolicy
import com.drclaude.data.AlertKind
...
import com.drclaude.data.HealthConnectManager
import com.drclaude.data.LibreLinkUpClient
import com.drclaude.data.MealReminderReceiver
import com.drclaude.data.Predictor
import com.drclaude.data.Trend
import com.drclaude.data.cleanGlucoseSeries
import com.drclaude.data.glucoseRisingNow
...
import com.drclaude.ui.AppHeader
import com.drclaude.ui.ConsentScreen
import com.drclaude.ui.DeviceRoleScreen
import com.drclaude.ui.RolePatient
import com.drclaude.ui.DashboardScreen
import com.drclaude.ui.AudioPlayingBanner
import com.drclaude.ui.NetworkBanner
import com.drclaude.ui.ServiceHealthBanner
import com.drclaude.ui.colorFor
import com.drclaude.ui.theme.AccentGreen
import com.drclaude.ui.theme.GlucoseStatus
import com.drclaude.ui.theme.statusOf
import com.drclaude.ui.Safety
import com.drclaude.ui.SafetyModal
import com.drclaude.ui.BatteryExemptionDialog
import com.drclaude.ui.HistoryScreen
import com.drclaude.ui.NotificationSettingsScreen
import com.drclaude.ui.Lang
import com.drclaude.ui.LocalStrings
import com.drclaude.ui.LoginScreen
import com.drclaude.ui.langFromCode
import com.drclaude.ui.stringsFor
import com.drclaude.ui.theme.DoctorClaudeTheme
...
import com.drclaude.ai.ApiAccess
import com.drclaude.ai.ClaimService
import com.drclaude.ai.ProfileService
import com.drclaude.ui.DoctorClaudeBottomBar
import com.drclaude.ui.FoodScreen
import com.drclaude.ui.InsulinScreen
import com.drclaude.ui.PatientChoice
import com.drclaude.ui.ProfileItem
import com.drclaude.ui.ProfileScreen
import com.drclaude.ui.Tab
...
class MainActivity : ComponentActivity() {
    ...
    private fun AppRoot() {
        ...
        // Record the meals / insulin tab should open its editor on, set by tapping that record's
        // marker on the glucose curve. Cleared by the screen once it has opened the dialog.
        var openMealId by remember { mutableStateOf<Long?>(null) }
        var openDoseId by remember { mutableStateOf<Long?>(null) }
        ...
        // A pinned device is held on its person, permanently. Login, a relaunch or a refresh can all
        // re-point the active patient at the account's default — on the child's phone that silently
        // reopens the parent's record, which is the exact hazard the pin exists to prevent.
        LaunchedEffect(state.patientId, state.connections, deviceRole) {
            val locked = store.lockedPatientId
            if (store.isChildDevice && locked != null && state.patientId != null && state.patientId != locked) {
                val nm = state.connections.firstOrNull { it.patientId == locked }
                    ?.let { "${it.firstName} ${it.lastName}".trim() }.orEmpty()
                repo.switchPatient(locked, nm)
            }
        }
        ...
        var homeEvents by remember { mutableStateOf<List<com.drclaude.ui.GraphEvent>>(emptyList()) }
        ...
        LaunchedEffect(state.current?.timestampMs, state.current?.valueMgDl, recentInsulinIob, iobReady) {
            ...
            // ONE decision function, shared with the background MonitorService — the UI used to carry
            // its own copy of these rules, and the copy had drifted: it re-rang on a timer, so simply
            // opening the app while high was enough to set it off. The bookkeeping is read FRESH from
            // the store (never from a remembered snapshot), because the service rewrites it while the
            // UI is not composing.
            val decision = AlarmEngine.decide(
                alert,
                AlarmEngine.Bookkeeping(
                    store.firedAlert, store.lastAlarmMs, store.lastAlarmValue, store.lastJudgedReadingMs
                ),
                nowA,
                readingMs = state.current?.timestampMs ?: 0L,
            )
            store.firedAlert = decision.book.firedKind
            store.lastAlarmMs = decision.book.lastAlarmMs
            store.lastAlarmValue = decision.book.lastAlarmValue
            store.lastJudgedReadingMs = decision.book.lastJudgedMs
            firedAlert = decision.book.firedKind
            if (decision.recovered) {
                if (ackAlert != null) { ackAlert = null; store.ackAlert = null }
                AlarmService.stop(this@MainActivity)
                return@LaunchedEffect
            }
            if (!decision.ring) return@LaunchedEffect
            ackAlert = null; store.ackAlert = null
            // The local ring is resolved against the user's notification settings (per-type on/off,
            // volume, vibrate, quiet hours). The red banner and the auto-spoken hypo analysis below
            // stay regardless of how the phone alarm is configured.
            ...
            // Auto-run the spoken analysis ONLY for a real hypo (LOW) — safety-critical, worth the
            // interruption. For a HIGH the alarm + banner are enough; we don't auto-replace the
            // report (the user disliked it "reloading a new analysis" by itself).
            if (alert.kind == AlertKind.LOW && store.voiceEnabled && state.patientId != null && store.aiReady) {
                ...
                lifecycleScope.launch {
                    try {
                        ...
                        // Fresh timestamp so the error is shown as current, not instantly aged out
                        // into the "refaire une analyse" prompt by the 20-min freshness rule.
        ...
        if (!consented) {
            ...
            DeviceRoleScreen(
                lang = lang,
                patients = state.connections.map {
                    RolePatient(it.patientId, "${it.firstName} ${it.lastName}".trim().ifBlank { it.patientId })
                }.sortedBy { it.label.lowercase() },
                onPickParent = {
                    store.deviceRole = CredentialsStore.ROLE_PARENT
                    store.lockedPatientId = null
                    deviceRole = store.deviceRole
                },
                onPickChild = { p ->
                    // Pin to the person the user NAMED, and move the app onto that record right away:
                    // the pin used to take whoever was active (the account's first person), so a phone
                    // set up for the child stayed on the parent's record for ever.
                    store.deviceRole = CredentialsStore.ROLE_CHILD
                    store.lockedPatientId = p.patientId
                    deviceRole = store.deviceRole
                    if (p.patientId != state.patientId) {
                        lifecycleScope.launch {
                            ai.stopSpeaking()
                            AlarmService.stop(this@MainActivity)
                            aiText = ""; aiLang = ""; autoGreeted = false
                            repo.switchPatient(p.patientId, p.label)
            ...
            Scaffold(bottomBar = {
                ...
                Box(Modifier.padding(pad)) {
                  Column(Modifier.fillMaxSize()) {
                    ...
                    ServiceHealthBanner()
                    // GLOBAL "audio is playing" bar with its STOP button — visible on EVERY tab, for
                    // EVERY source (spoken answer, photo scan, analysis, replay). The only stop
                    // control used to live inside a dashboard card that is often not on screen at
                    // all — a spoken answer never fills it, and the meals/insulin tabs, which START
                    // audio themselves, had no way to stop it.
                    AudioPlayingBanner(isSpeaking, lang) { ai.stopSpeaking() }
                    ...
                    if (showNotifSettings) {
                        ...
                        HistoryScreen(
                            ...
                            onEventClick = { e ->
                                ...
                                if (e.insulin) { openDoseId = e.id; tab = Tab.INSULIN }
                                else { openMealId = e.id; tab = Tab.MEALS }
                            },
                        )
                    } else {
                ...
                // A tap on a marker opens THAT record's editor on its own tab; a tap anywhere else
                // on the curve still opens the full history (onOpenHistory above).
                onEventClick = { e ->
                    showAllHistory = false
                    if (e.insulin) { openDoseId = e.id; tab = Tab.INSULIN }
                    else { openMealId = e.id; tab = Tab.MEALS }
                ...
                rescueRecent = rescueRecent,
                        ...
                        Tab.MEALS -> FoodScreen(
                            state.patientId, meals, ai, lang, state.history,
                            header = { tabHeader(stringsFor(lang).tabMeals, false) },
                            onDataChanged = { dataVersion++ },
                            openEditId = openMealId, onOpenEditConsumed = { openMealId = null },
                        )
                        ...
                        Tab.PROFILE -> ProfileScreen(
                            ...
                            patients = state.connections.map { c ->
                                PatientChoice(
                                    ...
                                    // Already in the connections payload — a pinned device can show
                                    // a parent's glucose without switching to (or fetching) anything.
                                    glucose = c.current?.valueMgDl,
                                    readingMs = c.current?.timestampMs ?: 0L,
                                )
                            }.sortedBy { it.label.lowercase() },
                            isChildDevice = store.isChildDevice,
                            onChangeRole = { if (!store.isChildDevice) deviceRole = null },
                            onSwitchPatient = { pid ->
...
private fun urgentQuestion(a: GlucoseAlert, lang: Lang, rescueRecent: Boolean = false): String {
    ...
    return when (a.kind) {
        AlertKind.LOW ->
            if (rescueRecent)
                (if (es) "Mi glucosa está en $v mg/dL, pero acabo de tomar azúcar hace unos minutos. ¿Espero a que actúe o tomo más?"
                 else "Ma glycémie est à $v mg/dL, mais je viens de prendre du sucre il y a quelques minutes. Je laisse agir ou j'en reprends ?")
            else if (es) "Urgencia: mi glucosa está en $v mg/dL (hipo). ¿Qué hago AHORA y cuántos terrones de azúcar?"
            else "Urgence : ma glycémie est à $v mg/dL (hypo). Que dois-je faire MAINTENANT et combien de morceaux de sucre ?"
        AlertKind.HIGH ->
        ...
        // The amber zones never auto-ask: they are a heads-up, not an emergency.
        AlertKind.LOW_WARN, AlertKind.HIGH_WARN, AlertKind.NONE -> ""
    }
}
