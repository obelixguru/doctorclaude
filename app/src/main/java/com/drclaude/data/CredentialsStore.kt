...
class CredentialsStore(context: Context) {
    ...
    /** Timestamp of the last glucose reading the alarm logic has already judged. An alert fires on a
     *  NEW reading only, so re-opening the app (or any re-evaluation of the same value) can never
     *  sound the alarm again — the "it rings every time I open it" bug. 0 = none judged yet. */
    var lastJudgedReadingMs: Long
        get() = prefs.getLong(KEY_LAST_JUDGED_READING, 0L)
        set(v) = prefs.edit().putLong(KEY_LAST_JUDGED_READING, v).apply()
    ...
    companion object {
        ...
        private const val KEY_LAST_JUDGED_READING = "last_judged_reading_ms"
