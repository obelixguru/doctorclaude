...
/** A meal or insulin event drawn as a dot ON the glucose curve. Tapping it shows a small preview. */
data class GraphEvent(val ts: Long, val insulin: Boolean, val label: String)
...
fun buildGraphEvents(
    ...
    for (m in meals) {
        ...
        out += GraphEvent(m.ts, false, label)
    }
    return out
}
                    ...
                    detectTapGestures { off ->
                        val cand = if (canvasSize.width == 0 || clusters.isEmpty()) null else {
                            ...
                            clusters
                                ...
                                .filter { it.third <= 44.0 }
                                .minByOrNull { it.third }
                        }
                        when {
                            cand != null -> { selected = cand.first; selectedPos = cand.second }
